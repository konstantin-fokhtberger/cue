...
| AUDIO-DEVICE-001        | EPIC-002 |       P0 | done        | Explicit cue input/output selectors and effective-device diagnostics                | ADR-DEVICE-001, TEST-AUDIO-001               |
...
| TEST-AUDIO-002          | EPIC-002 |       P0 | in_progress | Zoom/Teams/Meet x accepted audio-route automation matrix                            | SPIKE-AUDIO-001, AUDIO-DEVICE-001, CI-002    |
...
| UI-MENUBAR-001          | EPIC-009 |       P1 | done        | Add a menu bar status item with graceful `Close app` action                         | None                                         |
...
1. `TEST-AUDIO-002` - automate the feasible provider x route matrix and preserve Teams as
   unverified until a conference is available.
2. `UI-WINDOW-001` - add the accepted drag region.
3. `UI-MENUBAR-001` - add graceful menu bar shutdown with capture cleanup evidence.

The global capture-scope privacy risk, parent-death lifecycle gap and explicit-device
acceptance are closed. STT work can begin; meeting-assistance features still depend on the
session, timeline, and STT foundations.
...
- Microsoft Teams is waived only for the current manual spike because the product owner cannot
  ...
  passed.
- Deterministic source Electron coverage now exercises Zoom and browser-wide Chrome/Meet against
  built-in, Sony Bluetooth, HyperX USB + Sony Bluetooth, and explicit HyperX-over-Sony-default
  cue routes. These fixture cases prove UI/IPC/helper/Stop contracts only; they do not replace
  real CoreAudio, meeting-app selection, participant-count or transcript evidence.
